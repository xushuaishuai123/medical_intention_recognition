...
class Trainer:
    def __init__(self, model, config):
        ...
        self.device = self.config.device if self.config.device is not None else torch.device(
    ...
    def train(self, train_loader):
        ...
        self.model.train()
        self.best_acc = 0
        self.config.eval_steps = self.config.eval_steps if self.config.eval_steps > 1 else int(
        ...
        scheduler= self._get_scheduler(
            self.optimizer, scheduler='warmupcosine', warmup_steps=total_steps*0.01, t_total=total_steps
        )
...
def main():
    ...
    if args.do_predict:
        with open('./data/IMCS-DAC_test.json', 'r', encoding='utf-8-sig') as f:
            ...
            for key, value in tqdm(test_datas.items(),total=len(test_datas),desc='predicting'):
                # res_data = []
                for data in value:
        ...
        print('save predict to ./predict/IMCS-DAC_test.json')
